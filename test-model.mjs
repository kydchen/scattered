...
assert.match(app, /textContent = "E"[\s\S]*?addEventListener\("click", runViewportProbe\)/);
assert.match(app, /function runViewportProbe\(\)[\s\S]*?previousView[\s\S]*?screenToWorld[\s\S]*?EDIT_VIEW_SCALE[\s\S]*?applyView\(\)[\s\S]*?requestAnimationFrame[\s\S]*?Object\.assign\(board\.view, previousView\)/);
...
assert.match(serviceWorker, /scattered-v60/);
