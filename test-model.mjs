...
assert.match(stagePendingSaveSource, /syncOpenInputs\(\);[\s\S]*?if \(!boardDirty\) return;[\s\S]*?stagePendingDocument\(workspaceStorage, workspace, boardWithoutDragPreview\(\)\)/);
const dragSaveSource = app.match(/function boardWithoutDragPreview\(\) \{[\s\S]*?\n}/)[0];
...
assert.match(saveBoardNowSource, /saveDocument\(workspaceStorage, workspace, boardWithoutDragPreview\(\)\)/);
...
const menuButtonMarkup = html.match(/<button id="menu-button"[\s\S]*?<\/button>/)?.[0] || "";
assert.match(menuButtonMarkup, /aria-label="[^"]+"[\s\S]*?<svg[^>]*viewBox="0 0 24 24"[^>]*aria-hidden="true"[^>]*focusable="false"/);
assert.equal((menuButtonMarkup.match(/<circle\b/g) || []).length, 3);
...
assert.match(app, /applyDriveWorkspace[\s\S]*?fitIncoming[\s\S]*?replaceBoard\(applied\.nextBoard, applied\.fitIncoming, workspace\.activeId === previousActiveId\)/);
...
assert.match(app, /textContent = "F"[\s\S]*?addEventListener\("click", runViewportProbe\)/);
assert.match(app, /function runViewportProbe\(\)[\s\S]*?screenToWorld[\s\S]*?classList\.remove\("overview", "overview-compact", "overview-distant"\)[\s\S]*?translate3d[\s\S]*?setTimeout[\s\S]*?applyView\(\)[\s\S]*?1_000/);
...
assert.doesNotMatch(app, /scheduleViewportWake|wake:0|wake:raf|x \+ 0\.25/);
...
assert.match(app, /function replaceBoard\(nextBoard, fitIncoming = false, keepSelection = false\)[\s\S]*?applyView\(\)[\s\S]*?fitOpenedBoardIfOffscreen\(fitIncoming\)/);
...
assert.match(css, /#viewport\.overview \.node:active::before\s*\{[^}]*border-color:\s*var\(--thread\);[^}]*box-shadow:/s);
assert.match(css, /#viewport:not\(\.overview\) \.node:active\s*\{[^}]*box-shadow:\s*var\(--selected-shadow\);/s);
...
const editNodeSource = app.match(/function editNode\(id, isNew = false, fromPen = false\)[\s\S]*?\n}/)?.[0] || "";
...
assert.match(html, /<script type="module" src="app\.js\?v=82"><\/script>/);
assert.match(app, /from "\.\/sync-model\.js\?v=79"/);
assert.match(app, /from "\.\/drive-sync\.js\?v=81"/);
...
assert.match(serviceWorker, /scattered-v82/);
assert.match(serviceWorker, /\.\/svg-export\.js\?v=75/);
assert.match(serviceWorker, /\.\/app\.js\?v=82/);
...
assert.match(serviceWorker, /\.\/drive-sync\.js\?v=81/);
...
assert.match(syncConfig, /https:\/\/scatterednote\.space/);
assert.match(syncConfig, /https:\/\/sync\.scatterednote\.space/);
...
assert.match(html, /property="og:image" content="https:\/\/scatterednote\.space\/docs\/scattered-canvas\.png"/);
assert.match(html, /name="twitter:card" content="summary_large_image"/);
assert.match(html, /rel="canonical" href="https:\/\/scatterednote\.space\/"/);
