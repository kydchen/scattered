...
assert.match(app, /textContent = "D"[\s\S]*?addEventListener\("click", runViewportProbe\)/);
assert.match(app, /function runViewportProbe\(\)[\s\S]*?getComputedStyle\(element\)\.top[\s\S]*?top \+ 1[\s\S]*?setTimeout[\s\S]*?previousTops/);
...
assert.match(serviceWorker, /scattered-v59/);
