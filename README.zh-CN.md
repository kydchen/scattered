...
[在线使用（推荐）](https://scatterednote.space/) · [本地版](https://kydchen.github.io/scattered/) · [English](README.md)
...
它不要求注册账户，也没有模板或强制层级。默认情况下，画布保存在当前浏览器中，首次加载后也可以离线使用。推荐的在线版本可以在每台设备上授权一次 Google Drive，让整个工作区选择性地自动同步；每个 Google 账号拥有彼此隔离的工作区。[GitHub Pages 本地版](https://kydchen.github.io/scattered/)不含 Google 同步。两个版本都只会在你主动连接 Drive 或开启展示链接后上传相应内容；不开启分享，本地版仍完全本地使用。
...
- 把一张或多张选中卡片拖到另一张上即可连线，若全部已经相连则取消连线；覆盖时整组变半透明，松手后全部回到原位
- 使用 Pencil 圈选、鼠标框选，或手指长按空白后拖动框选，支持全选、整体移动和批量删除
...
- 清空后可以立即撤销，也可以预览最近五份恢复副本，并选择其中一份恢复为新画布
- 搜索卡片文字并依次跳转到结果
...
| 纯触控 | 长按空白后拖动框选，或长按卡片后点按补选；普通拖动平移、双指缩放 | 选择栏可一键全选，拖任意已选卡片可整体移动 |
...
启用可选的 Google Drive 同步并由用户主动连接后，工作区快照会从浏览器直接写入该用户 Drive 的隐藏应用数据目录。首次连接的账号会认领此前从未同步过的本地工作区；改用另一个账号时，浏览器会切换到该账号独立的本地工作区，不会合并或复制前一个账号的画布。断开连接只删除凭据，该账号的本地工作区仍可离线使用。OAuth 接口仅交换凭据；主动开启的展示链接使用独立的 Cloudflare D1 分享表，不会公开 Google Drive 文件。
...
- **JSON：**保留当前画布的可编辑内容。导入后会新增并打开画布，不会覆盖已有内容；名称重复时会自动添加后缀。旧版工作区备份中的画布也会加入列表。导入、导出位于左侧画布菜单；复制、删除按钮位于当前选中画布的右侧。
- **SVG：**把完整画布导出为轻量矢量图，并按照全部内容自动适配画布范围。
- **Mermaid Markdown：**把卡片和连线转换成可用于 Markdown 文档及 AI 协作流程的图表。
- **实时只读链接：**选择「导出 → 实时只读链接 → 开启分享」，复制链接即可发到投影仪电脑。对方无需登录，可以平移、缩放、聚焦和全屏展示，但不能编辑，也不会覆盖对方自己的本地工作区。仅发布当前画布。

保持开启分享的浏览器打开并联网，保存的内容修改通常会在几秒内更新到展示页，链接不变，也不会打断展示者的取景。作者离开后，最后成功发布的内容仍可查看。回到同一浏览器及工作区可以关闭分享；管理凭据只保存在该浏览器，不参与 Google 同步或导出，请在清除网站数据前先关闭分享。拿到链接的人可以转发或保存内容，关闭不能撤回已保存的副本。首版每张分享画布上限为 256 KiB、500 张卡片和 1,000 条连线。[部署及边界说明](docs/live-sharing.md)。
